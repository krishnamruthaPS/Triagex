...
// Base hospital data - Bangalore hospitals within ~50km of (13.032643, 77.5920005)
const baseHospitals = [
  {
...
// Additional cross-city hospitals (Mumbai / Delhi / Chennai etc.)
const additionalHospitals = [
  {
    name: "Manipal Hospital Whitefield", // duplicate candidate in base list
    email: "admin@manipal-whitefield.com",
    address: "#143, 212-2015, EPIP Industrial Area, Whitefield, Bangalore, Karnataka 560066",
    location: { type: "Point", coordinates: [77.7499, 12.9699] },
    contactPhone: "+91-80-6692-2222",
    services: ["Emergency", "Cardiology", "Neurology", "Oncology", "ICU"],
    isVerified: true
  },
  { // Mumbai
    ...
    location: { type: "Point", coordinates: [72.8198, 19.0596] },
  ...
  { // Mumbai
    ...
    location: { type: "Point", coordinates: [72.8064, 19.1136] },
  ...
  { // Delhi
    ...
    location: { type: "Point", coordinates: [77.2073, 28.5672] },
  ...
  { // Delhi
    ...
    location: { type: "Point", coordinates: [77.2420, 28.5355] },
  ...
  { // Chennai
    ...
    location: { type: "Point", coordinates: [80.2450, 13.0569] },
...
function buildUnifiedHospitalList() {
  const map = new Map();
  [...baseHospitals, ...additionalHospitals].forEach(h => {
    if (!h?.name) return;
    if (!map.has(h.name)) {
      map.set(h.name, h);
    }
...
async function populateHospitals() {
  try {
    ...
    const unified = buildUnifiedHospitalList();
    const before = unified.length;
    // Apply default password
    const defaultPassword = process.env.DEFAULT_HOSPITAL_PASSWORD || '12345';
    const pwdHash = await bcrypt.hash(defaultPassword, 10);
    const prepared = unified.map(h => ({
      ...h,
      password: pwdHash
    }));
    const insertedHospitals = await Hospital.insertMany(prepared);
    console.log(`Inserted ${insertedHospitals.length} hospitals (deduped from ${before} entries).`);
    console.log(`All hospitals share default password: ${defaultPassword}`);
    ...
    console.log('Login with hospital name exactly as stored + default password.');
